...
try:
    ...
    csv_file_path = './csv_files/year_counts.csv'
    df = pd.read_csv(csv_file_path)

    # Iterate through rows of the DataFrame and insert into MySQL database
    for index, row in df.iterrows():
        query = "INSERT INTO year_count (firm, year, year_count) VALUES (%s, %s, %s)"
        values = (
            row['firm'],
            None if pd.isna(row['year']) else row['year'],
            None if pd.isna(row['year_count']) else row['year_count']
        )
